...
export class RoleALoggedGuard extends BaseRole {
  ...
  public canActivate(route: ActivatedRoute): Observable<boolean|UrlTree> {
    return this.checkRole(Role.ROLE_LOGGED, route.snapshot.data?.roleGuardMeta ?? null);
  }
  ...
  public canActivateChild(childRoute: ActivatedRoute): Observable<boolean|UrlTree> {
    return this.checkRole(Role.ROLE_LOGGED, childRoute.snapshot.data?.roleGuardMeta ?? null);
  }
}
